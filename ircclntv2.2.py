...
names={}
root=Tk()
d = Myname(root,"Enter Username")
root.withdraw()
root.wait_window(d.top)
iprandom = IP(root,"Enter Address to connect to")
...
def recvfile():
    try:
        ...
        file=open(ext,'wb')
        global log
        log=log+'\n Recieving file from '+sern+';file-'+ext
        print("Recieving! wait..")
        while True:
            data=so.recv(512)
            if not data:
                break
